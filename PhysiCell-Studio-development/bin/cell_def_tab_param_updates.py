...
class CellDefParamUpdates:
    ...
    def cell_def_param_changed(self, text, check=True, name=None):
        ...
        if check:
            if name not in self.parent.param_d[self.get_current_celldef()].keys():
                ...
                # raise KeyError(f'cell_def_param_changed: {name} not in {self.parent.param_d[self.get_current_celldef()].keys()}')   # rwh pyside
    ...
    def cell_def_rate_changed(self, text, fn, time_step):
        # rwh pyside
        if self.parent.sender() == None:
            print("----- rwh: cell_def_rate_changed() error, return")
            return
        name = self.parent.sender().objectName()
    ...
    def cell_def_fixed_clicked(self, bval):
        # rwh pyside
        if self.parent.sender() == None:
            print("----- rwh: cell_def_fixed_clicked() error, return")
            return
        name = self.parent.sender().objectName()
